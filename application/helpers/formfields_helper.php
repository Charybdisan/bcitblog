 ...
 * @param string $name ID and name of the control; s/b the same as the RDB table column
 */
function makeBogusField($label, $name, $keep = TRUE) {
    $CI = &get_instance();
    $parms = array(
        'label' => $label,
        'name' => $name,
    );
    return $CI->parser->parse('_fields/bogusfield', $parms, $keep);
}
...
function makeSubmitButton_ImageUpload($label, $title, $css_extras = "", $keep = TRUE) {
    $CI = &get_instance();
    $parms = array(
        'label' => $label,
        'title' => $title,
    ...
    return $CI->parser->parse('_fields/submitImage', $parms, $keep);
}
...
/**
 * Make a rating display & voter. 
 * Note: this helper uses a CSS class rather than a DOM ID
 * 
 * @param <type> $label ID of the posting to be rated
 * @param <type> $name Class of the div to generate
 * @param <type> $value The current rating value
 */
function makeRatingVoter($label, $name, $value, $explain = "", $disabled = false, $keep = TRUE) {
    $CI = &get_instance(); // handle to CodeIgniter instance
    $CI->caboose->needed('rating', $name);
    $parms = array(
        'label' => $label,
        'name' => $name,
        'value' => $value,
    ...
    return $CI->parser->parse('_fields/rating', $parms, $keep);
}
